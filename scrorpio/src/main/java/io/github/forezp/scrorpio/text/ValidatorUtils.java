...
import io.github.forezp.scrorpio.exception.ScrorpioException;
import io.github.forezp.scrorpio.vo.ValidationResult;
    ...
    private static void checkError(ValidationResult result) {
        if (result.isHasErrors()) {
            ...
            if (result.getErrorMsg().size() > 0) {
                ...
                throw new ScrorpioException(ErrorCode.ERROR_ARGS, mapValueToString(result.getErrorMsg()));
            }
            throw new ScrorpioException(ErrorCode.ERROR_ARGS);
        }
    }
